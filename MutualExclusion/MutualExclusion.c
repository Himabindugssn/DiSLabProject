...
void *accept_thread(void *accept_sock){
    ...
    acpt_sock= ((int)(size_t)(accept_sock));
...
//             printf("\nNode: %d is sending message to enter into CS to Node: %d \n" , NODE_ID , i);
            printf("\nNode- %d is sending a Request_To_Enter_CS_Message to Node- %lld \n" , NODE_ID , i);
...
int main(int agrc , char *argc[]){
    ...
    san.sa_handler = (void (*)(int) ) signalHandler;
    ...
    sleep(3);
    if((err=pthread_create( ( pthread_t* )&tid[0] , NULL , &bind_thread , NULL))){
  ...
  while(counter){
    ...
    printf("================================================\n");
    printf("Node: %d is entering into critical section.\n" , NODE_ID);
      sleep(genNo(10));
